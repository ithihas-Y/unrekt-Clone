...
import { useCheckApproval, useFetchPoolsInfo, useFetchBalance, useFetchCurrentlyStaked, useFetchRewardsAvailable,useFetchRewardsAvailable2, useFetchHalfTime, useFetchCanWithdrawTime, useFetchApproval, useFetchStake,useFetchStake2, useFetchWithdraw,useFetchWithdraw2, useFetchClaim, useFetchExit } from '../redux/hooks';
...
export default function StakePool(props) {
  ...
  const { rewardsAvailable2, fetchRewardsAvailable2 } = useFetchRewardsAvailable2();
  ...
  const { fetchWithdraw, fetchWithdraw2, fetchWithdrawPending } = useFetchWithdraw();
  ...
  const [ myRewardsAvailable2, setMyRewardsAvailable2] = useState(new BigNumber(rewardsAvailable2[index]));
  ...
  useEffect(() => {
    const amount = byDecimals(rewardsAvailable2[index], pools[index].earnedToken2Decimals);
    setMyRewardsAvailable2(amount);
  }, [rewardsAvailable2[index], index]);
  ...
  useEffect(() => {
    if (address) {
      ...
      fetchRewardsAvailable2(index);
      if(Boolean(index === 5) || Boolean(index === 6)) fetchHalfTime(index);
      if(index === 5 || index === 6) fetchCanWithdrawTime(index);
  ...
  return (
    <Grid container style={{paddingTop: '4px', background: 'no-repeat center center url(' + bgImage + ')'}}>
      <Grid item xs={12}>
      ...
      <Grid item xs={12} className={classes.detailContentUp}>
        <GridContainer spacing={3} className={classNames({
          ...
          <GridItem sm={(index=== 3 || index===4) ? 4: 3} xs={12} className={classNames({
            ...
            <div className={classes.contentItemTitleVal}>{`${Math.floor(myRewardsAvailable2.toNumber() * 10000)/ 10000} ${pools[index].earnedToken2}`}</div>
                      ...
                      <div className={classes.stakeHintContainer}>
                        {
                          canWithdrawTimeIsMoreNowTime ? (
                            <span  >{t('Stake-Pool-Unstake-Hint-Date')+momentFormatTime(canWithdrawTime[index] * 1000)}</span>
                          ) : (
                            <span  >{t('Stake-Pool-Unstake-Hint')}</span>
                          )
                        }
                      </div>
                    ):(
