...
const _deposit = ({web3, contract, amount, isAll, address, dispatch}) => {
  return new Promise((resolve, reject) => {
    // console.log(isAll)
    if(isAll) {
      contract.methods.easyBuy1(amount).send({ from: address }).on('transactionHash', function(hash){
        console.log(hash)
      ...
      contract.methods.deposit().send({ from: address }).on('transactionHash', function(hash){
        console.log(hash)
        dispatch(enqueueSnackbar({
          message: hash,
          options: {
